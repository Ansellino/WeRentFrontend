...
export default function ShipmentPage() {
  ...
  const { data: options, isLoading } = useShipmentOptions(
    submitted ? address : ''
  )
    ...
    <div className='max-w-lg mx-auto space-y-6'>
      ...
      {options && (
        <div className='space-y-3'>
          ...
          {options.map(opt => (
            <button key={opt.id} onClick={() => setSelectedCourier(opt)}
              className={`w-full text-left border rounded-lg p-4 transition-colors ${
                checkout.selectedCourier?.id === opt.id
                  ? 'border-green-600 bg-green-50' : 'hover:bg-gray-50'
              }`}
            >
      ...
      <Button onClick={handleNext}
        disabled={!checkout.selectedCourier}

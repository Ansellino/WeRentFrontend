...
import { useProductDetail, useAvailability, useReviewSummary } from '@/lib/hooks/useProducts'
...
export default function ProductDetailPage() {
  ...
  const { data: availability, isFetching: checkingAvail } = useAvailability(
    id, { size: selectedSize, startDate, rentalDays }, availabilityEnabled
  )
  ...
  const handleAddToCart = () => {
    ...
    if (!selectedSize) { toast({ title: 'Select a size first', variant: 'destructive' }); return }
    if (!availability?.available) { toast({ title: 'Not available on selected dates', variant: 'destructive' }); return }
    ...
    <div className='space-y-8'>
        ...
        <div className='space-y-4'>
          ...
          <div className='grid grid-cols-2 gap-4'>
            ...
            <div>
              ...
              <input type='number' value={rentalDays} min={1} max={30}
          ...
          <div className='flex gap-3'>
            ...
            <Button variant='outline' onClick={() => toggleWishlist({ productId: id, inWishlist })}>
      ...
      <div>
        <div className='flex justify-between items-center mb-4'>
          ...
          <a href={`/products/${id}/reviews`} className='text-green-700 text-sm underline'>
            View all reviews
          </a>
        </div>
